...
public class RedMidDuckScoreAndPark extends SequentialCommandGroup {
    ...
    public RedMidDuckScoreAndPark(DrivetrainMecanum drivetrain, Arm arm, Intake intake, Telemetry telemetry, Carousel carousel) {
        ...
        addCommands(new ArmToPosition(arm, RobotMap.MID_TARGET, telemetry),
                new DriveDistance(drivetrain, 0.6, 9.0, telemetry).whenFinished(() -> drivetrain.stopAll()),
                new StrafeDistance(drivetrain, 0.6, 21.0, "LEFT", telemetry).whenFinished(() -> drivetrain.stopAll()),
                new DriveDistance(drivetrain, 0.4, -2.5, telemetry).whenFinished(() -> drivetrain.stopAll()),
                new CarouselDriveForward(carousel, telemetry).withTimeout(2000).whenFinished(() -> carousel.stopAll()),
                new StrafeDistance(drivetrain, 0.6, 53.0, "RIGHT", telemetry).whenFinished(() -> drivetrain.stopAll()),
                ...
                new DriveDistance(drivetrain, 0.6, -53.0, telemetry).whenFinished(() -> drivetrain.stopAll()),
